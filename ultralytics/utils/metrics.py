...
def bbox_iou_for_nms(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False,
    ...
    # ÐÞ¸´WIoU scaleÎÊÌâ
    if scale and WIoU:
        # ÕâÀïÐèÒªWIoU_ScaleÀàµÄÊµÏÖ£¬ÔÝÊ±×¢ÊÍµô
        # self = WIoU_Scale(1 - (inter / union))
        pass
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                rho_x = (s_cw / cw) ** 2
                rho_y = (s_ch / ch) ** 2
                gamma_siou = angle_cost - 2  # ÖØÃüÃû±ÜÃâÓë²ÎÊýgamma³åÍ»
...
def soft_nms(bboxes, scores, iou_thresh=0.45, sigma=0.3, score_threshold=0.30):
    """
    Soft-NMSÊµÏÖ - ÐÞ¸´°æ±¾
    Args:
        bboxes: ±ß½ç¿ò×ø±ê [N, 4]
        scores: ¼ì²â·ÖÊý [N]
        iou_thresh: IoUãÐÖµ
        sigma: Soft-NMSµÄsigma²ÎÊý
        score_threshold: ·ÖÊýãÐÖµ
    Returns:
        keep: ±£ÁôµÄË÷Òý
    """
    if bboxes.numel() == 0:
    ...
    while order.numel() > 0:
        ...
        # Ñ¡Ôñµ±Ç°µÃ·Ö×î¸ßµÄ¿ò
        max_score_idx = torch.argmax(scores[order])
        i = order[max_score_idx]
        keep.append(i.item())

        # ÐÞ¸´£ºÊ¹ÓÃÕýÈ·µÄ·½Ê½½»»»ÔªËØ
        if max_score_idx != 0:
            # ±£´æÒª½»»»µÄÖµ
            temp = order[0].clone()
            order[0] = order[max_score_idx]
            order[max_score_idx] = temp

        # ¼ÆËãÓëÆäËû¿òµÄIoU
        if order.numel() > 1:
            # »ñÈ¡µ±Ç°Ñ¡ÖÐ¿òºÍÆäÓà¿òµÄIoU
            current_box = bboxes[order[0]].unsqueeze(0)  # [1, 4]
            other_boxes = bboxes[order[1:]]  # [N-1, 4]

            iou = bbox_iou_for_nms(current_box, other_boxes, xywh=False)
            ...
            if valid_mask.any():
                # ±£ÁôÓÐÐ§µÄ¿ò£¨Ìø¹ýµÚÒ»¸öÒÑÑ¡ÖÐµÄ¿ò£©
                valid_indices = torch.where(valid_mask)[0] + 1  # +1ÒòÎªÌø¹ýÁËµÚÒ»¸öÔªËØ
                order = order[valid_indices]
            else:
                break
        else:
            break
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            ...
            if 'Pose' in str(save_dir):
                with open(f'D:\\Yolov\\ultralytics-8.2.0\\bsp\\pr\\{names[i]}_pose.csv', 'w+') as f:
                    for px_v, py_v in zip(px, y):
                        f.write(f'{px_v},{py_v}\n')
            else:
                with open(f'D:\\Yolov\\ultralytics-8.2.0\\bsp\\pr\\{names[i]}.csv', 'w+') as f:
                    for px_v, py_v in zip(px, y):
                        f.write(f'{px_v},{py_v}\n')
    else:
