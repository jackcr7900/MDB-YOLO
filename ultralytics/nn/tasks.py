...
from ultralytics.nn.modules import SimAM, ASPP
from ultralytics.nn.modules.DSConv import DSConv2D, C2f_DSConv
from ultralytics.nn.modules.block import SimSPPF,FocalModulation
from ultralytics.nn.Extramodule.ODConv import *
from ultralytics.nn.backbone.EffcientViM import C2f_EfficientViM
...
# from ultralytics.nn.EMA import C2f_EMA
...
from ultralytics.mymodels.C2f_EMA import C2f_EMA
from ultralytics.nn.Extramodule import SPDConv
from ultralytics.nn.modules import lska_module
...
class BaseModel(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        """
        if isinstance(x, dict):  # for cases of training and validating wFhile training.
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3,C2f_SCConv,C2f_DSConv,C2f_Faster,C2f_EfficientViM,C2fGhost,C3STR,FasterNet,C2f_DCN}:  # update channels
        ...
        elif m in (CBAM, my_CBAM, CPCA):
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
        elif m is EnhancedCBAM:
            c1, c2 = ch[f], args[0]  # c1: 输入通道数, c2: 目标通道数
            if c2 != nc:  # 如果 c2 不是类别数，则调整通道数
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # 检查 args 长度，支持可选参数
            if len(args) > 1:
                ratio = args[1]  # 降维比例
                kernel_size = args[2] if len(args) > 2 else 7  # 卷积核大小，默认 7
                act = args[3] if len(args) > 3 else nn.ReLU  # 激活函数，默认 ReLU
                norm_layer = args[4] if len(args) > 4 else None  # 归一化层，默认 None
                args = [c1, ratio, kernel_size, act, norm_layer]
            else:
                args = [c1]  # 仅提供 in_planes，使用默认参数
        elif m in {EMA}:
            args = [ch[f],*args]
        elif m is CARAFE:
            c1 = ch[f]
    ...
    #         m_ = m
    #         m_.backbone = True
    #         t = str(m)[8:-2].replace('__main__.', '')  # ✅ 增加这一行：获取模块类型字符串
    #     else:
