...
class my_CBAM(nn.Module):
    def __init__(self, c1, kernel_size=7):
        ...
        self.channel_attention = my_channel_attention(c1)   # 通道注意力
        self.spatial_attention = my_spacial_atteition(kernel_size)  # 空间注意力
...
class StdPool2d(nn.Module):
    ...
    def forward(self, x):
        # Compute standard deviation across H and W dimensions
        std = torch.std(x, dim=(-2, -1), keepdim=True)
        # Expand to match output_size (e.g., 1x1)
        return std.expand(-1, -1, self.output_size, self.output_size)
...
class PConv(nn.Module):
    def __init__(self, c1, c2, k, s):
        ...
        self.cw = Conv(c1, c2 // 4, (1, k), s=s, p=0)  # 输出 c2//4
        self.ch = Conv(c1, c2 // 4, (k, 1), s=s, p=0)  # 输出 c2//4
        self.cat = Conv(c2, c2, 2, s=1, p=0)  # 输入 c2，输出 c2
...
class RFAConv(nn.Module):  # »ùÓÚGroup ConvÊµÏÖµÄRFAConv
    ...
    def forward(self, x):
        ...
        feature = self.generate_feature(x).view(b, c, self.kernel_size ** 2, h,
                                                w)  # b c*kernel**2,h,w ->  b c k**2 h w   »ñµÃ¸ÐÊÜÒ°¿Õ¼äÌØÕ÷
