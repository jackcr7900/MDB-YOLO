// ODConv2d TensorRT Plugin Header
// 基于 ultralytics/nn/Extramodule/ODConv.py

#ifndef ODCONV_PLUGIN_H
#define ODCONV_PLUGIN_H
...
class ODConvPlugin : public IPluginV2DynamicExt {
public:
    ODConvPlugin(int in_channels, int out_channels, int kernel_size,
                 int stride, int padding, int groups, int kernel_num);
    ODConvPlugin(const void* data, size_t length);
    ~ODConvPlugin() override;
...
private:
    ...
    float* d_attention_weights{nullptr};
};

class ODConvPluginCreator : public IPluginCreator {
public:
    ODConvPluginCreator();
    ~ODConvPluginCreator() override = default;
